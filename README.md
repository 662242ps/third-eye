# Third Eye 
https://third-eye-landing.vercel.app/
...
- เลือก **Camera index** ได้จากหน้านี้ ค่าเริ่มต้นคือ `0` และจะมีผลเมื่อเปิดกล้องครั้งถัดไป
- **Focal Length**: ค่าปรับเทียบกล้อง ใช้ร่วมกับความสูงโดยประมาณของวัตถุ
- ปรับความสูงโดยประมาณของ `รถยนต์`, `รถบรรทุก`, `รถจักรยานยนต์` และ `คน` ได้เป็นเมตร
- ค่าเริ่มต้นเป็นค่าประมาณ จึงควรปรับเทียบกับกล้อง มุมกล้อง และวัตถุจริงที่ใช้งาน

ตัวช่วย **Calculate focal length** ใช้ค่าระยะจริง, ความสูงกรอบวัตถุในภาพ (พิกเซล) และความสูงจริงของวัตถุ เพื่อประมาณค่า Focal Length ตามสูตร:

```text
Focal Length = ระยะจริง × ความสูงกรอบวัตถุ / ความสูงจริงของวัตถุ
```

ควรวัดจากวัตถุที่อยู่ตรงกลางภาพและรู้ระยะจริง แล้วทดสอบซ้ำกับวิดีโอจริงก่อนใช้งาน
...
ในหน้า **ตั้งค่า > โมเดลตรวจจับ** สามารถปรับค่าได้ดังนี้:

- `conf` (Confidence) ช่วง 0.01–0.99: ค่าต่ำจะรับวัตถุได้มากขึ้น แต่อาจมีการตรวจจับผิดมากขึ้น; ค่าสูงจะกรองวัตถุที่ไม่มั่นใจออกมากขึ้น
- `IoU` (NMS) ช่วง 0.01–0.99: ใช้ควบคุมการกรองกรอบวัตถุที่ซ้อนกัน ค่าสูงจะยอมให้กรอบซ้อนกันได้มากขึ้น
- ค่าเริ่มต้นคือ `conf=0.50` และ `IoU=0.55` และบันทึกไว้ใน `settings/model_settings.json`

การเปลี่ยนเฉพาะ `conf` หรือ `IoU` จะมีผลกับการตรวจจับทันทีโดยไม่โหลดโมเดลใหม่ ส่วนการเปลี่ยนไฟล์โมเดลจะโหลดโมเดลและทำ warm-up ใหม่
...
ระบบจะพูดซ้ำวัตถุและระดับความเสี่ยงเดิมได้ไม่ถี่กว่า 8 วินาที และต้องมีระยะเปลี่ยนอย่างน้อย 5 เมตร (`vision/voice_alert.py`)
...
### 3.6 คืนค่าการตั้งค่า

ปุ่ม **Reset defaults** ที่ด้านล่างหน้าตั้งค่าจะคืนค่า ระยะเตือน, โซน, กล้อง, Focal Length, ความสูงวัตถุ, โมเดล และเสียงกลับเป็นค่าเริ่มต้น หลังยืนยันแล้วระบบจะบันทึกและนำค่าไปใช้ทันที
...
สร้าง segment ใหม่ด้วยเสียงผู้ชายที่ติดตั้งไว้:

```powershell
python tools/generate_voice_segments.py --voice-model th_m_1 --force
```

คำสั่งนี้ใช้โมเดล ONNX เฉพาะตอนสร้างไฟล์เท่านั้น การตรวจจับจริงยังเล่น WAV ที่สร้างไว้และไม่โหลดโมเดลเสียง
...
- แถบสถานะด้านล่างจะแสดง `display FPS`, `AI latency`, `skipped` และ `read_errors` เพื่อช่วยแยกว่าอาการกระตุกเกิดจากการแสดงผล, AI ช้า, ตัวถอดรหัสข้ามเฟรม หรืออ่านเฟรมผิดพลาด
- `skipped` ที่เพิ่มต่อเนื่องหมายถึงระบบกำลังประมวลผลไม่ทันแหล่งภาพ จึงเลือกเฟรมล่าสุดเพื่อป้องกันคิวสะสมและความหน่วง
- `read_errors` ที่เพิ่มขึ้นหมายถึงปัญหาจากไฟล์วิดีโอ, codec, กล้อง หรือ backend ของ OpenCV ควรทดสอบไฟล์/อุปกรณ์อื่น
- สถานะ `AI=CPU` หรือ `AI=GPU` บอกอุปกรณ์ที่ตรวจพบจริง ปัจจุบันระบบจะเลือก CUDA อัตโนมัติเมื่อ Torch และไดรเวอร์รองรับ มิฉะนั้นจะ fallback เป็น CPU
...
### วิดีโอค้างเป็นช่วง ๆ

- ดูค่า `AI latency`: ถ้าสูงต่อเนื่อง แสดงว่าโมเดลใช้เวลาประมวลผลมากกว่าช่วงเวลาของเฟรม
- ดูค่า `skipped`: ถ้าเพิ่มเร็ว แสดงว่าเครื่องตามวิดีโอไม่ทัน ระบบจะทิ้งเฟรมเก่าเพื่อให้ภาพไม่หน่วงสะสม
- ดูค่า `read_errors`: ถ้าเพิ่ม ให้ตรวจ codec/ไฟล์วิดีโอและลองแปลงเป็น H.264 MP4
- เสียงพูดใช้ WAV ที่เตรียมไว้ล่วงหน้าและเล่นใน worker แยก จึงไม่ควรบล็อก UI หรือหยุดลูปวิดีโอ
